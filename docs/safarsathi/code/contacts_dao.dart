...
// The dialer never touches the database directly; it watches these streams.
//
...
/// Trust tier. Drives visual treatment in the dialer — see DESIGN.md.
...
class ContactCategory {
  ...
  static const all = [
    emergency, hospital, pharmacy, accommodation, restaurant,
    transport, guide, fuel, localContact, embassy, other,
  ];
...
class ContactsDao extends DatabaseAccessor<AppDatabase> with _$ContactsDaoMixin {
  ...
  /// Main dialer feed. Ordering is deliberate: pinned first, then the
  /// contacts you've actually confirmed, then everything else by name.
  /// Recently-called floats up within each group.
  ...
  Stream<int> watchUnconfirmedCount(int tripId) {
    ...
    final q = selectOnly(contacts)
      ...
      ..where(contacts.tripId.equals(tripId) &
          contacts.callConfirmed.equals(false));
    ...
    await transaction(() async {
      ...
      await this.batch((b) {
        b.insertAll(
          contacts,
          entries
              .map((e) => e.copyWith(importBatchId: Value(batchId)))
              .toList(),
        );
      });
    });
  ...
  Future<void> rollbackImport(int batchId) async {
    await transaction(() async {
      await (delete(contacts)..where((c) => c.importBatchId.equals(batchId)))
          .go();
  ...
  Future<void> togglePin(int id, bool pinned) =>
      (update(contacts)..where((c) => c.id.equals(id)))
          .write(ContactsCompanion(isPinned: Value(pinned)));
  ...
  Future<void> markConfirmed(int id, {bool confirmed = true}) =>
      (update(contacts)..where((c) => c.id.equals(id))).write(
        ContactsCompanion(
          ...
          tier: Value(confirmed
              ? ContactTier.userVerified.name
              : ContactTier.userEntered.name),
        ),
      );

  /// Called on every dial/WhatsApp/SMS action. Feeds the recents ordering
  /// and gives the user a record of who they actually reached.
  Future<void> logCall({
    ...
    await transaction(() async {
      await into(callLogs).insert(CallLogsCompanion.insert(
        contactId: contactId,
        tripId: Value(tripId),
        action: action,
      ));
      final row = await (select(contacts)..where((c) => c.id.equals(contactId)))
          .getSingleOrNull();
      if (row != null) {
