...
Path _ticketPath(Size size, TicketEdge edge) {
  ...
  void notchedEdge(double y, bool leftToRight) {
    final count = (size.width / pitch).floor();
    for (var i = 0; i < count; i++) {
      final x = leftToRight ? (i + 0.5) * pitch : size.width - (i + 0.5) * pitch;
      path.arcToPoint(
        Offset(leftToRight ? x + r : x - r, y),
        radius: const Radius.circular(r),
        clockwise: leftToRight ? (y != 0) : (y == 0),
      );
      path.lineTo(
        leftToRight ? (i + 1.5) * pitch - r : size.width - (i + 1.5) * pitch + r,
        y,
      );
    }
  }
    ...
    return Semantics(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          final t = Curves.elasticOut.transform(_ctl.value.clamp(0.0, 1.0));
          // -8deg settling to -3deg, ink bleeding in to 0.85.
          final degrees = -8 + 5 * t;
          return Opacity(
            opacity: (_ctl.value * 0.85).clamp(0.0, 0.85),
