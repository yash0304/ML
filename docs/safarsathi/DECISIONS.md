...
2026-09-24 — [DISCOVERY] **Sights are downloaded by default**: tourism=viewpoint/attraction, waterway=waterfall and natural=cave_entrance, labelled "Sights". They are last in the query order, so a slow download has already fetched fuel and hospitals. (Nohkalikai is tagged as a waterfall and Mawsmai as a cave entrance, not as viewpoints; asking only for viewpoints would miss what the Sohra road is driven for.)

2026-09-24 — [TRIP] **Each stop records where you actually sleep (Stops.stayContactId, schema v6). Saved is not chosen.** The Tonight card, the plan sent home and the readiness check use only the chosen stay. With none chosen they say "not decided" or ask you to choose, even when a single option is saved, because a sheet of options puts one at most stops. A stay you type as the first one at a stop is adopted, and an imported one never is. You choose from the Tonight card, the stop's page, the guest house's page, or by tapping a readiness item. **Supersedes 2026-09-13's "one confirmed number clears the stop"**: a confirmed number for a guest house you are not staying at clears nothing. (Found on the phone: Bramhome Guest House showed as the Shillong bed because it sorted first, and the person is not staying there.)

2026-09-24 — [DATA] Stops.stayContactId has no foreign key. A backup restores stops before contacts, so a reference would fail every restore with a stay chosen. A deleted contact leaves an id that matches nothing, and every reader treats that as not decided. Additive migration, tested from a real v5 file, and a v5 backup (no key) still restores.

2026-09-23 — [CONTACTS] **A diary entry's page shows where it is: an offline map with the place pinned, "How far am I?" from the phone's GPS (straight line, said as such), and Directions handed to Google Maps from wherever the phone is.** The app draws no turn-by-turn route, because that needs a routing server and the app makes no network call on the road. The handoff says it needs signal *before* the tap, and says a Google Maps offline area removes that need. An entry with no position offers a Google search by name, labelled as a search. (Asked for from the phone: "from this screen I should have a map so that I can have a direction from my current location.")

2026-09-23 — [CONTACTS] **A position comes from the person, never from geocoding.** It is pasted from Google Maps (press-and-hold numbers, degrees-minutes-seconds, or a long link, where the `!3d!4d` pin beats the `@` camera), or taken from "I am here now". A `maps.app.goo.gl` short link is refused with what to do instead, because it holds no coordinates. A location that won't read blocks the save; unlike a bad number, dropping it silently would leave the person believing the place is mapped. (A guessed point would sit a hospital at the wrong kilometre on every leg.)

2026-09-23 — [IMPORT] **Re-importing a sheet that gained Latitude/Longitude adds the position to diary entries with the same number and no position, ticked by default, with no second copy.** Only the position is written, and only where `lat IS NULL`. Name, note, tier and confirmation stay the person's. A sheet that only places makes no empty batch, and undoing a batch leaves added positions in place (the preview says so). (27 numbers "had no location" on the phone because the first sheet had none; the newer file's rows were all flagged duplicates and left unticked, so the positions never landed.)

2026-09-23 — [IMPORT] Numbers with no E.164 form (112, 181, 1098) are duplicates when the name and digits both match. (They had no duplicate check at all: a simulated re-import of the real sheet would have added ten helplines a second time.) Also fixed: ticking a preview row off and on dropped its position.

2026-09-23 — [TRIP] Short codes and 1800 toll-free lines are not counted as "numbers with no location": they are services, not places, and a sheet does not always mark them as emergency numbers.
